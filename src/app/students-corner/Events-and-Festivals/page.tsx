...
import {
  CalendarHeart, ExternalLink, Mail, Target,
...
function EventsAndFestivalsContent() {
  ...
  useEffect(() => {
    const fetchFestivals = async () => {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('category', 'Events & Festivals')
        .eq('status', 'Active')
        .order('display_order', { ascending: true });
      if (!error && data && data.length > 0) {
        setFestivals(data as FestivalItem[]);
  ...
  useEffect(() => {
    const param = searchParams.get('event');
    if (param && festivals.find(c => c.slug === param)) setActiveSlug(param);
  ...
  const active = festivals.find(c => c.slug === activeSlug) ?? festivals[0];
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans">
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white">Events &amp; Festivals</h1>
            </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-32 text-gray-400">
          <Loader2 className="animate-spin mr-2" size={24} /> Loading festivals...
        </div>
      ) : festivals.length === 0 ? (
          ...
          <div className="w-full md:w-1/3 lg:w-1/4 shrink-0">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sticky top-24">
              ...
              <div className="flex flex-col p-2 max-h-[70vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                {festivals.map((f) => (
                  <button key={f.slug} onClick={() => handleSelect(f.slug)}
                    className={`text-left px-4 py-3 rounded-xl transition-all duration-200 flex items-center gap-3 font-semibold text-sm ${
                      activeSlug === f.slug ? "bg-[#123B6D] text-white shadow-md" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                    }`}>
                    <CalendarHeart size={16} className={activeSlug === f.slug ? "text-blue-200" : "text-gray-400"} />
              ...
              <AnimatePresence mode="wait">
                <motion.div key={active.slug} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3 }}
                  ...
                  <div className="p-8 md:p-10 border-b border-gray-100 flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                    <div className="w-20 h-20 rounded-2xl flex items-center justify-center shrink-0 bg-purple-100 text-purple-600 border border-purple-200">
                      <CalendarHeart size={36} strokeWidth={2} />
                    </div>
                    <div className="flex-1 w-full">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <h2 className="text-3xl md:text-4xl font-black text-gray-900">{active.name}</h2>
                        <Link href={`/students-corner/gallery?department=${encodeURIComponent(active.name)}`}
