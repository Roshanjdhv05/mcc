...
import { ShieldCheck, ChevronLeft, FileText, Calendar, ExternalLink } from 'lucide-react';
...
const accreditationNav = [
  ...
  { label: 'AISHE', href: '/accreditation/aishe/annual-submissions', active: true },
];

const aisheSubmissions = [
  {
    year: '2024–25',
    status: 'Submitted',
    description: 'Annual AISHE data submission for academic year 2024–25.',
  },
  {
    year: '2023–24',
    status: 'Submitted',
    description: 'Annual AISHE data submission for academic year 2023–24.',
  },
  {
    year: '2022–23',
    status: 'Submitted',
    description: 'Annual AISHE data submission for academic year 2022–23.',
  },
  {
    year: '2021–22',
    status: 'Submitted',
    description: 'Annual AISHE data submission for academic year 2021–22.',
  },
];

const aisheStats = [
  { label: 'Total Students', value: '2,850+' },
  { label: 'Programmes Offered', value: '12' },
  { label: 'Faculty (Full-time)', value: '68' },
  { label: 'Years Submitted', value: '4+' },
];

export default function AisheAnnualSubmissionsPage() {
    ...
    <div className="bg-[#F8FAFC] min-h-screen pb-12 font-sans">
      ...
      <div className="relative py-10 flex flex-col items-center text-center overflow-hidden">
        ...
        <div className="inline-flex items-center gap-2 bg-[#123B6D]/10 border border-[#123B6D]/20 text-[#123B6D] px-4 py-1.5 rounded-full text-xs font-bold mb-4 uppercase tracking-widest">
          <ShieldCheck size={13} /> AISHE
        </div>
        <h1 className="text-3xl lg:text-4xl font-black text-[#123B6D] tracking-tight mb-2">
          AISHE Annual Submissions
        </h1>
        <p className="text-gray-500 text-sm max-w-xl px-4 mb-6">
          All India Survey on Higher Education annual data submissions by Mulund College of Commerce.
        </p>
      ...
      <div className="max-w-4xl mx-auto px-4 lg:px-8 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {aisheStats.map((stat, i) => (
            <div key={i} className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm p-5 text-center">
              <div className="text-2xl font-black text-[#123B6D]">{stat.value}</div>
        ...
        <div className="bg-[#123B6D] rounded-2xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-lg mb-6">
          <div>
            <h2 className="text-base font-bold text-white">AISHE Data Portal</h2>
            <p className="text-white/70 text-sm mt-0.5">View MCC's official submissions on the Ministry of Education's AISHE portal.</p>
          </div>
          <a
          ...
          >
            <ExternalLink size={15} /> Visit AISHE Portal
          </a>
        </div>

        {/* Yearly Submissions */}
        {aisheSubmissions.map((sub, i) => (
          <div
            key={i}
