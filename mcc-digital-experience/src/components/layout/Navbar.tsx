        ...
        sections: [
          {
            links: [
              ...
              { label: "MCC's Calendar", href: '/iqac/reports-and-initiatives?tab=academic-calendar' },
...
export default function Navbar() {
  ...
  const fetchLiveNotices = async () => {
    ...
    const { data: jrData } = await supabase
      ...
      .select('id, title, schedule_time, show_on_home, category, attachments, expiry_time')
      .lte('schedule_time', now)
      .or(`expiry_time.is.null,expiry_time.gt.${now}`)
      .order('schedule_time', { ascending: false })
      .limit(10);
    ...
    let combined = [...(data || [])];
    if (jrData) {
      const mappedJrData = jrData.map((doc: any) => ({
        id: `jr-${doc.id}`,
        title: doc.title,
        schedule_time: doc.schedule_time,
        courses: ['jr-college'],
        categories: [doc.category],
        is_general: doc.show_on_home,
        attachments: doc.attachments || [],
      }));
      combined = [...combined, ...mappedJrData];
    }

    combined.sort((a, b) => new Date(b.schedule_time).getTime() - new Date(a.schedule_time).getTime());
    const topNotices = combined.slice(0, 10);

    if (topNotices && topNotices.length > 0) {
      setLiveNotices(topNotices);
      const latestId = topNotices[0].id;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'jr_college_notices' },
        (payload) => {
          console.log('Realtime event received! New jr college notice:', payload);
          fetchLiveNotices();
        }
      )
      .subscribe((status) => {
              ...
              <div className="flex items-center gap-1.5 md:gap-2 lg:gap-5 shrink-0 ml-auto">
                ...
                <div className="hidden xl:flex flex-col items-end gap-0 w-max">
                  <div className="flex items-center gap-1.5 bg-blue-50 border border-blue-100 px-3 py-1 rounded-full">
                    <Users size={14} className="text-[#123B6D]" />
                    <span className="text-[13px] font-bold text-[#123B6D]">Visitors: {visitorCount.toLocaleString()}</span>
                  </div>
                </div>
                ...
                {/* Admission Button — between trust logo and search */}
                <Link
                  href="/admission"
                  className="hidden md:flex items-center justify-center h-8 md:h-9 lg:h-10 px-4 lg:px-6 rounded-full bg-gradient-to-r from-red-600 to-red-700 text-white font-bold text-[10px] md:text-xs lg:text-sm shadow-md hover:shadow-lg hover:scale-105 transition-all shrink-0"
                >
                  Admission
                </Link>
