...
import {
  ...
  Loader2, LayoutGrid, AlertCircle
} from 'lucide-react';
...
export default function EventsList({ refreshKey }: { refreshKey: number }) {
  ...
  const fetchEvents = async () => {
    ...
    const { data, error } = await supabase
      .from('events')
      .select('id, title, category, department, publish_gallery, publish_home, publish_calendar, calendar_date, calendar_type, status, published_at, images')
            ...
            <tr key={ev.id} className="hover:bg-gray-50/60 transition-colors">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span title="Academic Calendar" className={`w-7 h-7 rounded-lg flex items-center justify-center ${ev.publish_calendar ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-300'}`}>
                    <BookOpen size={13} />
                  </span>
                </div>
              </td>
