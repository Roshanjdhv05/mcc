...
export default function FormsPageClient() {
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen relative">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 -mt-8 pb-16 space-y-6 relative z-10">
        {/* Search & Filter */}
          ...
          {forms.map((f, i) => (
            <div key={i} className="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm p-5 hover:shadow-md hover:-translate-y-0.5 transition-all flex items-center gap-4">
              ...
              {f.href && f.href !== '#' ? (
                ...
                <button 
                  onClick={() => {
                    if (f.title === 'Railway Concession Form') {
                      setShowRailwayModal(true);
                    }
                  }}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#123B6D]/10 text-[#123B6D] text-xs font-semibold hover:bg-[#123B6D] hover:text-white transition-all flex-shrink-0"
                >
