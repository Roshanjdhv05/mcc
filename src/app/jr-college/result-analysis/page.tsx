...
const jrCollegeNav = [
  ...
  { label: 'RESULT ANALYSIS', href: '/jr-college/result-analysis', active: true },
...
export default function ResultAnalysisPage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY > lastScrollY && currentScrollY > 200) setNavVisible(false);
          ...
          {jrCollegeNav.map((item, i) => (
            <Link
              ...
              className={`flex-shrink-0 h-full flex items-center px-4 lg:px-5 text-[11px] lg:text-xs font-bold transition-colors uppercase whitespace-nowrap tracking-wider ${
                item.active ? 'bg-[#008e59] text-white' : 'text-white/90 hover:text-white hover:bg-white/10'
              }`}
            >
              {item.label}
            </Link>
            ...
            {[...jrCollegeNav, ...jrCollegeNav, ...jrCollegeNav].map((item, i) => (
              <Link
                ...
                className={`flex-shrink-0 h-full flex items-center px-5 text-[11px] font-bold transition-colors uppercase whitespace-nowrap tracking-wider border-r border-white/10 ${
                  item.active ? 'bg-[#008e59] text-white' : 'text-white/90 active:text-white active:bg-white/10'
                }`}
