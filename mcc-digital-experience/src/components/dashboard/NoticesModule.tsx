...
import { useCachedNotices, useCachedExamDocs } from '@/hooks/useCachedSupabase';
...
export default function NoticesModule({ courseCode }: { courseCode: string }) {
  ...
  const loading   = noticesLoading || examLoading;
  ...
  const handleRefresh = useCallback(async () => {
    ...
    await Promise.all([refetchNotices(), refetchExamDocs()]);
    setRefreshing(false);
  }, [qc, refetchNotices, refetchExamDocs]);

  // Map exam docs into notice shape
  const examNotices: Notice[] = (rawExamDocs as any[]).map((doc: any) => ({
    id: `exam-${doc.id}`,
    title: doc.title,
