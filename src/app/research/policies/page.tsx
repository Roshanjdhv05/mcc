...
import {
  FileCheck, ShieldAlert, FileSearch, ChevronRight, Loader2, Download, Info, CheckCircle2
} from "lucide-react";
...
function PoliciesContent() {
  ...
  const handleSelect = (id: string) => {
    ...
    router.push(`/research/policies?tab=${id}`, { scroll: false });
  };
            ...
            >
              ...
              <div className="p-6 md:p-10 space-y-10">
                ...
                {activeTab === "research-policy" && (
                  <section>
                    <div className="flex items-center gap-2 text-[#123B6D] font-bold text-lg mb-4">
                      <Info size={20} /> About the Policy
                    </div>
                    <p className="text-gray-600 leading-relaxed text-base md:text-lg whitespace-pre-line mb-8">
                      Our Research Policy provides a framework for conducting ethical, high-quality research. It outlines the responsibilities of researchers, support systems provided by the institution, and guidelines for publications and intellectual property.
                    </p>
                    <div className="bg-[#F8FAFC] border border-gray-100 rounded-xl p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center">
                          <FileCheck size={24} />
                        </div>
                        <div>
                          <p className="font-bold text-[#1E293B]">Research Policy Document</p>
                          <p className="text-sm text-gray-500">PDF Document • 2.4 MB</p>
                        </div>
                      </div>
                      <button className="flex items-center gap-2 bg-[#123B6D] hover:bg-[#0D2A4F] text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors">
                ...
                {activeTab === "plagiarism-policy" && (
                  <section>
                    <div className="flex items-center gap-2 text-[#123B6D] font-bold text-lg mb-4">
                      <ShieldAlert size={20} /> Academic Integrity
                    </div>
                    <p className="text-gray-600 leading-relaxed text-base md:text-lg whitespace-pre-line mb-8">
                      The Plagiarism Policy strictly prohibits the unauthorized use of another&apos;s ideas, words, or data without proper attribution. All research submitted must pass stringent similarity checks using university-approved software.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-gray-50 border border-gray-100 rounded-xl p-5 flex items-start gap-4">
                         <div className="text-red-500 mt-1"><ShieldAlert size={20} /></div>
                         <div>
                           <p className="font-bold text-[#1E293B] mb-1">Zero Tolerance</p>
                           <p className="text-sm text-gray-600">Strict disciplinary action against intentional academic misconduct.</p>
                         </div>
                      </div>
                      <div className="bg-gray-50 border border-gray-100 rounded-xl p-5 flex items-start gap-4">
                         <div className="text-green-600 mt-1"><CheckCircle2 size={20} /></div>
                         <div>
                           <p className="font-bold text-[#1E293B] mb-1">Permissible Limit</p>
                           <p className="text-sm text-gray-600">Similarity index must be below 10% excluding standard exclusions.</p>
                         </div>
                      </div>
                    </div>
                    <div className="mt-8 bg-[#F8FAFC] border border-gray-100 rounded-xl p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-red-100 text-red-600 flex items-center justify-center">
                          <ShieldAlert size={24} />
                        </div>
                        <div>
                          <p className="font-bold text-[#1E293B]">Plagiarism Policy Document</p>
                ...
                {activeTab === "application-check" && (
                  <section>
                    <div className="flex items-center gap-2 text-[#123B6D] font-bold text-lg mb-4">
                      <FileSearch size={20} /> How to Apply for a Check
                    </div>
                    <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 mb-8">
                      <p className="text-gray-600 leading-relaxed">
                        To request a formal plagiarism check for your thesis, dissertation, or research paper, please fill out the application form and submit it to the Research Committee office along with your document in digital format.
                      </p>
                      <ul className="mt-4 space-y-3 text-gray-600">
                        <li className="flex items-start gap-3">
                          ...
                          <span>Download the Application Form below.</span>
                        </li>
                        <li className="flex items-start gap-3">
                          <div className="w-6 h-6 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 mt-0.5">
                            <span className="text-sm font-bold">2</span>
                          </div>
                          <span>Fill it completely and get it signed by your assigned Research Guide.</span>
                        </li>
                        <li className="flex items-start gap-3">
                          <div className="w-6 h-6 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 mt-0.5">
                            <span className="text-sm font-bold">3</span>
                          </div>
                          <span>Submit the soft copy of your document (Word/PDF without bibliography/references) via email to the library.</span>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="bg-[#F8FAFC] border border-gray-100 rounded-xl p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-[#1E293B]">Application Form</p>
                          <p className="text-sm text-gray-500">PDF Document • 450 KB</p>
                        </div>
                      </div>
                      <button className="flex items-center gap-2 bg-[#123B6D] hover:bg-[#0D2A4F] text-white px-5 py-2.5 rounded-lg text-sm font-semibold transition-colors">
                        <Download size={16} /> Download
