...
import { StudentsCornerItem } from './StudentsCornerManager';
import {
...
interface FullItem extends StudentsCornerItem {
...
interface StudentsCornerEditorProps {
  item: StudentsCornerItem;
  isNew: boolean;
  onClose: () => void;
}
...
// ─── Main Editor ──────────────────────────────────────────────────────────────
export default function StudentsCornerEditor({ item, isNew, onClose }: StudentsCornerEditorProps) {
  ...
  useEffect(() => {
    if (!isNew && item.id) {
      supabase.from('mcc_students_corner').select('*').eq('id', item.id).single().then(({ data }) => {
        if (data) {
          ...
          setObjectives(Array.isArray(data.objectives_activities) ? data.objectives_activities : []);
  ...
  const handleSave = async () => {
    ...
    // Validate contacts: at least one contact must have phone or email
    ...
    const { error } = isNew
      ? await supabase.from('mcc_students_corner').insert([payload])
      : await supabase.from('mcc_students_corner').update(payload).eq('id', item.id);

    if (error) showMsg('error', error.message);
    else {
      cacheLog('INVALIDATED', 'mcc_students_corner', 'save action');
      qc.invalidateQueries({ queryKey: qk.studentsCorner() });
      qc.invalidateQueries({ queryKey: qk.studentsCorner(formData.category) });
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-white border border-gray-200 rounded-2xl px-6 py-4 shadow-sm sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">{formData.category || 'Students Corner'}</p>
          </div>
        </div>
      ...
      <Section title="Basic Information" icon={<AlignLeft size={15} />}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            ...
            <input name="name" type="text" value={formData.name || ''} onChange={handleChange} className={inputClass} placeholder="e.g. Students' Council" />
          </div>
          <div>
            <label className={labelClass}>URL Slug <span className="text-red-500">*</span></label>
            <input name="slug" type="text" value={formData.slug || ''} onChange={handleChange} className={inputClass} placeholder="e.g. students-council" />
          </div>
          <div>
            <label className={labelClass}>Category <span className="text-red-500">*</span></label>
            <select name="category" value={formData.category || 'Forums and Clubs'} onChange={handleChange} className={inputClass}>
              <option value="Forums and Clubs">Forums and Clubs</option>
              <option value="Events & Festivals">Events &amp; Festivals</option>
              <option value="Student's Publications">Student's Publications</option>
            </select>
          </div>
        ...
        <div className="mt-5">
          ...
          <div className="flex gap-5 items-start mt-1">
            ...
            <div>
              <p className="text-xs text-gray-500 mb-3">Upload a banner image displayed at the top of this page (recommended: 1200×400px).</p>
      ...
      <Section title="About" icon={<AlignLeft size={15} />}>
        ...
        <textarea
          ...
          placeholder="Write a detailed description about this club, festival, or publication..."
          ...
          {committee.map((m, i) => (
            <div key={i} className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 bg-gray-50 rounded-xl p-4 border border-gray-100 relative group">
              <div>
                ...
                <input type="text" value={m.name} onChange={e => updateMember(i, 'name', e.target.value)} className={inputClass} placeholder="e.g. Rohan Mehta" />
              </div>
              <div>
                <label className={labelClass}>Role / Designation</label>
                <input type="text" value={m.role} onChange={e => updateMember(i, 'role', e.target.value)} className={inputClass} placeholder="e.g. President" />
              </div>
              <div>
      ...
      <Section title="Objectives & Activities" icon={<Target size={15} />}>
        <p className="text-xs text-gray-500 mb-4">Add content as free-form paragraphs or structured bullet points.</p>
        <div className="space-y-3">
          {objectives.map((o, i) => (
      ...
      <Section title="Contact Us" icon={<Phone size={15} />}>
        <p className="text-xs text-gray-500 mb-4">
          Add contact persons for this club/publication. Each contact <strong>must have at least an email or phone number</strong>.
        </p>
        <div className="space-y-3">
          {contacts.map((c, i) => (
            <div key={i} className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-gray-50 rounded-xl p-4 border border-gray-100 relative group">
              <div>
                <label className={labelClass}>Name <span className="text-red-500">*</span></label>
                <input type="text" value={c.name} onChange={e => updateContact(i, 'name', e.target.value)} className={inputClass} placeholder="e.g. Prof. Kavita Iyer" />
              </div>
              <div>
                <label className={labelClass}>Email</label>
