...
import { Search, Filter, Download, Bell, ChevronRight, X, Globe, Clock, Calendar, CalendarDays, FileText, Image as ImageIcon, FileIcon, RefreshCw, Check, ExternalLink } from 'lucide-react';
...
// Splits text into plain-text segments and URLs, renders URLs as clickable links
          ...
          return (
            <span key={i}>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1 text-[#123B6D] font-medium underline underline-offset-2 hover:text-[#D4A017] break-all transition-colors"
                onClick={e => e.stopPropagation()}
              >
                <ExternalLink size={11} className="shrink-0 inline" />
                {clean.length > 50 ? clean.slice(0, 50) + '…' : clean}
              </a>
              {trailing}
            </span>
          );
...
export default function NoticesPage() {
  ...
  const [selectedDate, setSelectedDate] = useState<{ day: string; month: string; year: string }>({ day: '', month: '', year: '' });
  ...
  const fetchNotices = useCallback(async (isRefresh = false) => {
    ...
    // Map jr college notices into notice shape
    const jrNotices: Notice[] = (jrNoticesData || []).map((doc: any) => {
      return {
        id: `jr-${doc.id}`,
        title: doc.title,
        ...
        departments: [],
        semesters: [],
        is_general: doc.show_on_home, // True if shown on home
        schedule_time: doc.schedule_time,
    ...
    const combined = [...(regularNotices as Notice[] || []), ...examNotices, ...jrNotices]
  ...
  useEffect(() => {
    ...
    const examChannel = supabase
      ...
      .subscribe();
    // Real-time: jr college notices
    const jrNoticeChannel = supabase
      .channel('public:jr_college_notices')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'jr_college_notices' },
        () => { fetchNotices(); }
      )
      .subscribe();
    return () => {
      ...
      supabase.removeChannel(jrNoticeChannel);
    };
  }, [fetchNotices]);
  ...
  const filtered = notices.filter(n => {
    ...
    // Date match
    const noticeDate = new Date(n.schedule_time);
    const matchDay = !selectedDate.day || noticeDate.getDate() === parseInt(selectedDate.day);
    const matchMonth = !selectedDate.month || (noticeDate.getMonth() + 1) === parseInt(selectedDate.month);
    const matchYear = !selectedDate.year || noticeDate.getFullYear() === parseInt(selectedDate.year);
    
    return matchSearch && matchCat && matchCourse && matchDay && matchMonth && matchYear;
  });
  ...
  const activeFilterCount = selectedCategories.length + selectedCourses.length +
    (selectedDate.day || selectedDate.month || selectedDate.year ? 1 : 0);
            ...
            {filtered.map((n) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Description */}
                  {n.description && (
                    <p className={`text-sm text-[#64748B] leading-relaxed mb-3 ${isExpanded ? '' : 'line-clamp-3'}`}>
                      <LinkifiedText text={n.description} />
                    </p>
                  )}
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-5 pb-24">
              ...
              <div className="mb-8">
                <h3 className="font-semibold text-[#1E293B] mb-3 flex items-center gap-2">
                  <CalendarDays size={16} className="text-[#123B6D]" /> By Date
                </h3>
                <div className="grid grid-cols-3 gap-2">
                  {/* Day */}
                  <div>
                    <label className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider block mb-1">Day</label>
                    <select
                      value={tempDate.day}
                      onChange={e => setTempDate(d => ({ ...d, day: e.target.value }))}
                      className="w-full border border-[#E2E8F0] rounded-lg px-2 py-2 text-sm text-[#1E293B] bg-white focus:outline-none focus:ring-2 focus:ring-[#123B6D]/30"
                    >
                  ...
                  {/* Month */}
                  <div>
                    <label className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider block mb-1">Month</label>
                    <select
                      value={tempDate.month}
                      onChange={e => setTempDate(d => ({ ...d, month: e.target.value }))}
                      className="w-full border border-[#E2E8F0] rounded-lg px-2 py-2 text-sm text-[#1E293B] bg-white focus:outline-none focus:ring-2 focus:ring-[#123B6D]/30"
                    >
                      <option value="">Any</option>
                  ...
                  {/* Year */}
                  <div>
                    <label className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider block mb-1">Year</label>
                    <select
                      value={tempDate.year}
                      onChange={e => setTempDate(d => ({ ...d, year: e.target.value }))}
                      className="w-full border border-[#E2E8F0] rounded-lg px-2 py-2 text-sm text-[#1E293B] bg-white focus:outline-none focus:ring-2 focus:ring-[#123B6D]/30"
                    >
                      <option value="">Any</option>
                ...
                {(tempDate.day || tempDate.month || tempDate.year) && (
                  <button
                    onClick={() => setTempDate({ day: '', month: '', year: '' })}
