...
export default function CourseTemplate({ title, shortInfo, fundingType, introductionContent, syllabusContent, quickActionsData: customQuickActionsData, courseKey, facultyData, festivals, publication }: CourseTemplateProps) {
  ...
  // Direct independent faculty state — fetched separately to avoid useProgramme Promise.all schema cache issues
  const [directFaculty, setDirectFaculty] = useState<any[] | null>(null);

  // Department filters (BCOM only)
  const BCOM_DEPT_ORDER = ['All', 'IKS', 'Business Economics', 'Economics', 'Commerce', 'Accountancy', 'English and Indian Languages', 'Law', 'Business Law', 'Environmental Studies', 'Mathematics and Statistics'];
  ...
  // Fetch faculty directly and independently to guarantee all rows are loaded
  useEffect(() => {
    if (!targetSlug) return;
    let cancelled = false;
    async function fetchFacultyDirect() {
      const { data: prog } = await supabase
        .from('mcc_programmes').select('id').eq('slug', targetSlug).single();
      if (!prog || cancelled) return;
      const { data: fac } = await supabase
        .from('program_faculty')
        .select('id, sr_no, name, designation, additional_role, department, education, teaching_exp, email, image, display_order, linkedin_url, research_interests, corporate_exp')
        .eq('programme_id', prog.id)
        .order('display_order');
      if (!cancelled) {
        console.log('[DirectFaculty] fetched:', fac?.length, fac?.map((f: any) => f.name));
        setDirectFaculty(fac || []);
      }
    }
    fetchFacultyDirect();
          ...
          {activeTab === 'Overview' ? (
            ...
            <div className="bg-white rounded-3xl p-6 md:p-10 border border-[#E2E8F0] shadow-sm">
              ...
              {(() => {
                ...
                if (cKey === 'BBA' || cKey === 'BCOM-BA') cKey = 'BCOM_BA';
                if (cKey === 'BMS' || cKey === 'BCOM-MS') cKey = 'BCOM_MS';
                if (cKey === 'BSC-DS' || cKey === 'DS') cKey = 'BSC_DS';
                if (cKey === 'BSC-CS') cKey = 'BSC_CS';
                if (cKey === 'BSC-IT') cKey = 'BSC_IT';
                if (cKey === 'BSC-CA') cKey = 'BSC_CA';
                ...
                if (isBcom) {
                  if (facultyDeptFilter === 'All') {
                    depts.filter(d => d !== 'All').forEach(dept => {
                      if (deptData[dept]) frontendFaculty.push(...deptData[dept]);
                    });
                    Object.keys(deptData).forEach(key => {
                      if (!depts.includes(key)) frontendFaculty.push(...deptData[key]);
                    });
                  } else {
                    frontendFaculty = deptData[facultyDeptFilter] || [];
                  }
                } else {
