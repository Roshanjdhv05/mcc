...
import { ChevronDown, ChevronRight, FileText, Download, ExternalLink, ClipboardList, AlertCircle } from 'lucide-react';
...
export default function AqarPage() {
  const [openCycles, setOpenCycles] = useState<string[]>(['2023-2024']);
  const [activeItem, setActiveItem] = useState<string>('23-24-report');
  ...
  const allItems = aqarSidebar.flatMap(c => c.items);
  const activeItemData = allItems.find(i => i.id === activeItem);
  const activeLabel = activeItemData?.label ?? 'AQAR Report';

  const selectItem = (id: string) => {
    setActiveItem(id);
    setMobileSidebarOpen(false);
    if (window.innerWidth < 1024) {
      document.getElementById('aqar-content')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  };
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen font-sans">
      ...
      <div className="bg-gradient-to-br from-[#123B6D] to-[#1a5499] pt-20 pb-12 relative overflow-hidden">
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 relative z-10">
          <nav className="flex items-center gap-2 text-white/60 text-xs font-semibold mb-5">
            ...
            <span>/</span>
            <span className="text-[#D4A017]">AQAR</span>
          </nav>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight">Annual Quality Assurance Reports</h1>
              <p className="text-white/70 text-sm mt-1">Year-wise AQAR documents submitted to NAAC</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 lg:px-12 mt-8 mb-12 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        {/* Content Pane */}
        <div id="aqar-content" className="lg:col-span-3">
